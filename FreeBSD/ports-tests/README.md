...
| `hermes-agent_test.sh` | `misc/hermes-agent`          | FreeBSD-only. `pkg add`s the freshly-built package and exercises the four salvage-patch pieces plus a real LLM round-trip: (1) the three `bin/` wrappers resolve and `hermes --help` returns 0 (sys.path shim); (2) `hermes skills list` shows a non-empty catalog (PR#31850 freebsd->linux skill mapping — used to be 0); (3) `allow_lazy_installs` resolves to False in a fresh FreeBSD config (the config_defaults.py patch, Commit B); (4) rc.d lifecycle — `service hermes_gateway {start,status,stop}` drive the port-installed rc.d script (Commit A); (5) a real single-turn completion via hermes' `custom` OpenAI-compatible provider against a local llama.cpp server (default `192.168.100.8:8080`, model `Agents-A1-MTP-Q8_0`, `CUSTOM_BASE_URL`/`CUSTOM_API_KEY` + `--provider custom --cli --yolo -z`), asserting non-empty output and no provider error. HOME redirected to a tempdir so real `~/.hermes` is untouched; LLM step SKIPs if the server is unreachable. Override with `LLAMA_URL`/`LLAMA_MODEL`. Trap cleans up rc.conf, sysrc, and the package. | sudo, `curl`, `hermes-agent-*.pkg` in poudriere; optional llama.cpp backend |

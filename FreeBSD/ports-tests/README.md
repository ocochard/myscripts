...
| `pimd_test.sh`      | `net/pimd`                      | 5-jail vnet PIM-SM lab (ED1 — R1 — R2 — R3 — ED2, static unicast routing, R2 is BSR + RP) with `start`/`check`/`run`/`stop`. Covers the FreeBSD code paths upstream CI never runs: pimd's own test suite is Linux-only (netns + veth + `unshare`), and everything here goes through `routesock.c` RPF lookups and the `kern.c` BSD branches instead of `netlink.c`. Asserts PIM adjacency over the epairs, the RP set reaching every router from the BSR, real end-to-end multicast (ED1 → group → ED2, counting `mping` replies so PIM's convergence loss is not a failure), and that `pimctl show mrt` agrees with the kernel MFC in each vnet. **Sets `net.inet.ip.mcast.loop=0` on the host** (restored by `stop`): `phyint_send()` otherwise loops every forwarded packet back into `ip_input`, pimd answers the resulting wrong-iif upcall with an Assert, loses the election against itself, and installs an MFC entry with an empty oif list — 4 of 60 packets delivered instead of 40 of 40. The sysctl is a global, not VNET-ized, so it cannot be set per jail. Needs a built pimd tree (`PIMD_SRC`, default `~/pimd`) rather than the package, since the port still tracks 2.3.2b. | `sudo`, root, VIMAGE kernel, `ip_mroute.ko`, built pimd in `$PIMD_SRC` |
...
# net/pimd — PIM-SM multicast lab in vnet jails, from a built pimd tree
sh pimd_test.sh run                       # start + check + stop, non-zero on failure
PIMD_SRC=~/pimd sh pimd_test.sh start     # or drive it by hand
sudo jexec pimd_r2 ~/pimd/src/pimctl -u /tmp/pimd-test/r2.sock show pim detail
sh pimd_test.sh check                     # logs kept in /tmp/pimd-test.failed on failure
sh pimd_test.sh stop                      # also restores net.inet.ip.mcast.loop

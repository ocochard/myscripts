...
#
echo "Station hostname;Station MAC;Protocol Used;Connected to AP;Received Signal Strength Indicator;Signal-to-Noise Ratio;Quality (RSSI/SNR
);WLAN SSID;STATUS"
snmpwalk $args ${bsnMobileStationMacAddress} | while read line; do
        ...
        # A first test if station still connected
        [ "${RSSI}" = "No Such Instance currently exists at this OID" ] && continue
        ...
        # resolve APMAC->NAME
        # A last test if station still connected
        [ "${APMAC}" = "NoSuchInstancecurrentlyexistsatthisOID" ] && continue
